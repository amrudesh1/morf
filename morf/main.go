...
func runServer(cmd *cobra.Command, args []string) {
	...
	// SCALE-1: construct the storage backend (local/object-store, env-driven).
	// TODO(SCALE-1): wire `store` into the upload handler / worker pool in a
	// later wave; router.InitRouters signature is left unchanged here to avoid
	// editing other files.
	if store, err := storage.NewFromEnv(); err != nil {
		log.Warnf("Failed to initialize storage backend: %v", err)
	} else {
		log.Infof("Storage backend initialized: %T", store)
	}
...
func runAPIOnly(cmd *cobra.Command, args []string) {
	...
	// SCALE-1: construct the storage backend (local/object-store, env-driven).
	// TODO(SCALE-1): wire `store` into the upload handler in a later wave;
	// router.InitRouters signature is left unchanged here to avoid editing
	// other files.
	if store, err := storage.NewFromEnv(); err != nil {
		log.Warnf("Failed to initialize storage backend: %v", err)
	} else {
