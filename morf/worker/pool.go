...
// mixes I/O wait with heavy CPU/JVM-bound work (apktool + apkanalyzer
// decompilation dominate), so it is NOT purely I/O-bound: oversubscribing 2x
// risks JVM memory thrash and CPU contention. The default oversubscription
// factor is therefore a modest 1.25: default = max(2, round(EffectiveCPUs *
// ioFactor)).
//
...
//	MORF_IO_FACTOR         — CPU oversubscription factor (default 1.25)
//	MORF_WORKER_MEM_BUDGET_MB — total memory budget for all in-flight scans;
//	                            when set, caps worker count to floor(budget/per-scan)
//	MORF_SCAN_MEM_MB       — per-scan memory estimate (defaults to a value derived
//	                          from MORF_JVM_MAX_HEAP_MB, so setting only the budget
//	                          above is enough to activate the cap)
func NewWorkerPool(q *queue.JobQueue) *WorkerPool {
	effectiveCPUs := utils.EffectiveCPUs()

	// MED-ioFactor: tunable oversubscription factor, defaulting low (1.25)
	// because decompilation is CPU/JVM-bound rather than pure I/O wait.
	ioFactor := envFloat("MORF_IO_FACTOR", 1.25)
	...
	// SC-3: optional memory-budget cap: cap = floor(budget / per_scan), min 2.
	// The per-scan estimate now has a sensible NON-zero default derived from the
	// per-JVM heap bound (MORF_JVM_MAX_HEAP_MB, default 2048): a scan's memory
	// peak is dominated by the two apktool JVMs that decompile sources and
	// resources in parallel, so ~2 * heap + overhead. With a non-zero default,
	// setting only MORF_WORKER_MEM_BUDGET_MB is enough to cap the worker count
	// (previously the cap silently did nothing unless MORF_SCAN_MEM_MB was also
	// set). The total budget remains opt-in: a safe default cannot be guessed
	// without reading the cgroup memory limit (see residual).
	if memBudget := envInt("MORF_WORKER_MEM_BUDGET_MB", 0); memBudget > 0 {
		jvmHeapMB := envInt("MORF_JVM_MAX_HEAP_MB", 2048)
		defaultScanMem := 2*jvmHeapMB + 512
		if perScan := envInt("MORF_SCAN_MEM_MB", defaultScanMem); perScan > 0 {
