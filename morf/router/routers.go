...
// scanResultSchemaVersion stamps every job result so consumers (frontend,
// webhook subscribers) can branch on schema shape during evolution.
...
func checkExternalToolsHealth() gin.H {
	...
	check("apktool", "java", "-jar", "/app/tools/apktool.jar", "--version")
...
func InitRouters(router *gin.RouterGroup) *gin.RouterGroup {
	...
	// AUTH-1: optionally require an API key on all data routes registered below.
	// The probe/observability endpoints above (/metrics, /health, /ready, /live)
	// intentionally stay public. Default off so existing keyless clients are not
	// broken; operators opt in with MORF_REQUIRE_API_KEY=true. The middleware
	// caches valid keys (TTL) and flushes last-used updates async/batched, so it
	// adds no synchronous DB write per request.
	if os.Getenv("MORF_REQUIRE_API_KEY") == "true" {
		router.Use(auth.APIKeyAuth())
		log.Info("API key auth ENABLED on /api data routes (MORF_REQUIRE_API_KEY=true)")
	}
	...
	router.POST("/upload", func(c *gin.Context) {
		...
		// Save job to Redis. On failure (after a successful Put), delete the
		// stored object so nothing leaks.
		if err := queue.GetQueue().CreateJob(job); err != nil {
			if delErr := store.Delete(ctx, storageKey); delErr != nil {
				log.WithFields(log.Fields{
					"request_id":  requestID,
					"storage_key": storageKey,
					"error":       delErr.Error(),
				}).Warn("Failed to clean up stored object after CreateJob error")
			}
			log.WithFields(log.Fields{
				"request_id": requestID,
				"job_id":     jobID,
		...
		// Push job to queue. On failure, delete the stored object so nothing leaks.
		if err := queue.GetQueue().PushJob(jobID); err != nil {
			if delErr := store.Delete(ctx, storageKey); delErr != nil {
				log.WithFields(log.Fields{
					"request_id":  requestID,
					"storage_key": storageKey,
					"error":       delErr.Error(),
				}).Warn("Failed to clean up stored object after PushJob error")
			}
			log.WithFields(log.Fields{
				"request_id": requestID,
				"job_id":     jobID,
				"error":      err.Error(),
			}).Error("Failed to push job to queue")
			c.JSON(http.StatusInternalServerError, gin.H{
	...
	router.POST("/bulk-upload", func(c *gin.Context) {
		...
		for _, file := range files {
			...
			// Save job to Redis. On failure, delete the stored object so nothing leaks.
			if err := queue.GetQueue().CreateJob(job); err != nil {
				_ = store.Delete(ctx, storageKey)
				errors = append(errors, fmt.Sprintf("%s: Failed to create job: %s", file.Filename, err.Error()))
				continue
			}
