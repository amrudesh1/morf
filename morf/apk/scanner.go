...
func StartSecScanE(ctx context.Context, apkPath string, jobCtx *utils.JobContext) ([]models.SecretModel, error) {
	...
	go func() {
		...
		defer cancel()
		_, sourceError = utils.RunWithContext(srcCtx, "java", "-jar", "/app/tools/apktool.jar", "d", "-r", apkPath, "-o", jobCtx.GetSourceDir())
	...
	go func() {
		...
		defer cancel()
		_, resError = utils.RunWithContext(resCtx, "java", "-jar", "/app/tools/apktool.jar", "d", "-s", apkPath, "-o", jobCtx.GetResDir())
...
// InvalidatePatternCache forces the pattern cache to be rebuilt on the next
// scan. Call this after patterns are added/removed/edited out-of-band.
...
func scBuildPatternCache(files []fs.FileInfo, maxMod time.Time, jobID string) (*scPatternCache, error) {
	...
	for _, file := range files {
		...
		yamlFile := readPatternFile(patternsDir + "/" + name)
...
func StartScanE(ctx context.Context, jobCtx *utils.JobContext) ([]models.SecretModel, error) {
	...
	onLine := func(b []byte) error {
		...
		if matched == nil {
			// Could not determine the pattern at all; fall back to the first.
			matched = &cache.patterns[0]
		}
...
func SanitizeSecrets(scanner_data []models.SecretModel) []models.SecretModel {
	...
	uniqueSecrets := make(map[string]models.SecretModel, len(scanner_data))
