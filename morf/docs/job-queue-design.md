...
- **Type:** List (RPush to tail / blocking head pop)
- **Purpose:** FIFO queue of job IDs waiting to be processed
- **Operations:**
  - `RPUSH morf:job_queue {jobID}` - Enqueue: append job to the TAIL of the queue
  - `BLMOVE morf:job_queue morf:processing:{workerID} LEFT RIGHT {timeout}` -
    Reliable blocking dequeue: atomically pop from the HEAD of `morf:job_queue`
    (LEFT) and push to the TAIL of the worker's processing list (RIGHT). RPush
    (tail) + head pop yields FIFO order.

> **Reliable, at-least-once delivery.** Workers never destructively pop. They use
> `BLMOVE` to atomically relocate a job from the queue head into a per-worker
> processing list, so an in-flight job is never lost if the worker crashes. The
> job is removed from the processing list only when the worker acknowledges it
> (see Processing Lists below). A dead-worker reaper recovers orphaned entries.

### 1a. Processing Lists (`morf:processing:{workerID}`)
- **Type:** List (one per worker)
- **Purpose:** Holds the jobs a given worker has dequeued but not yet acknowledged,
  providing reliable at-least-once recovery.
- **Operations:**
  - `BLMOVE morf:job_queue morf:processing:{workerID} LEFT RIGHT {timeout}` -
    Dequeue: job lands here while being processed.
  - `LREM morf:processing:{workerID} 1 {jobID}` - **Ack**: remove the job after it
    is fully processed (success, failure, or DLQ). `RequeueProcessing` performs the
    same `LREM` (count 1) before re-pushing to `morf:job_queue` for recovery, so a
    single occurrence can never be double-queued.
- **Recovery:** The dead-worker reaper (`StartReaper`) periodically scans worker
  registrations; for any worker whose `last_heartbeat` is older than the stale
  threshold it requeues every entry in that worker's processing list back onto
  `morf:job_queue` and deletes the dead registration. The `LREM`-before-`RPush`
  ordering keeps requeue idempotent even if two reapers sweep the same worker.
...
4. Enqueue to tail: `RPUSH morf:job_queue {jobID}`

### Processing a Job
1. Worker reliably dequeues: `BLMOVE morf:job_queue morf:processing:{workerID} LEFT RIGHT {timeout}` (job now held in the worker's processing list)
...
7. **Ack** (always, terminal): `LREM morf:processing:{workerID} 1 {jobID}` to remove the job from the processing list. If the worker dies before this Ack, the reaper requeues the job (`RequeueProcessing`), so processing must be idempotent.
...
- The dead-worker reaper (`StartReaper`) requeues every job in a dead worker's
  `morf:processing:{workerID}` list back onto `morf:job_queue` (via
  `RequeueProcessing`, which `LREM`s before `RPush` so requeue is idempotent),
  then deletes the dead worker registration.
